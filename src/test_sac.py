#!/usr/bin/env python3
"""
Test script for SAC-trained Crazyflie model
...
# SAC Policy Network (same as training)
...
def load_model(checkpoint_path):
    ...
    try:
        ...
        # Create policy
        policy = PolicyNetwork(state_dim=18, action_dim=4, hidden_dim=256).to(device)
        policy.load_state_dict(checkpoint['actor_state_dict'])
        policy.eval()
        
        print(f"Loaded model from episode {checkpoint.get('episode', 'unknown')}")
        print(f"Training reward: {checkpoint.get('reward', 'unknown')}")
        print(f"Difficulty level: {checkpoint.get('difficulty', 'unknown')}")
...
def test_model(policy, num_episodes=20, max_steps=1000, visualize=True):
        ...
        for episode in range(num_episodes // len(test_configs)):
            ...
            for step in range(max_steps):
                with torch.no_grad():
                    ...
                    _, _, action = policy.sample(state_tensor)
                ...
                reward -= linear_velocity * 2.0                 # Velocity penalty
                
                quat = data.qpos[3:7]
                quat = quat / np.linalg.norm(quat)
                upright_bonus = quat[0] ** 2
    ...
    for config_name, results in all_results.items():
        print(f"\n{config_name}:")
        for r in results:
            if r['metric'] in ['reward', 'steps', 'avg_error', 'time_precise', 'time_good']:
    ...
    if easy_results['std'] > 0.001:
        ...
        print("RIGID: Limited behavioral diversity")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test SAC Crazyflie Model')
    ...
    try:
        ...
        if args.model:
            ...
            # Look for best SAC model first
            if os.path.exists('models/crazyflie_sac_best.pth'):
                checkpoint_path = 'models/crazyflie_sac_best.pth'
            else:
                # Find latest SAC checkpoint
                sac_checkpoints = [f for f in os.listdir('models') 
                                 if f.startswith('crazyflie_sac_checkpoint_') and f.endswith('.pth')]
                if sac_checkpoints:
                    latest = max(sac_checkpoints, key=lambda x: int(x.split('_')[-1].split('.')[0]))
                    checkpoint_path = f"models/{latest}"
                else:
                    print("No SAC models found!")
                    return
        ...
        print(f"\nTesting SAC model with {args.episodes} episodes...")
