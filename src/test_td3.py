#!/usr/bin/env python3
"""
Test script for TD3-trained Crazyflie model
...
# TD3 Actor Network (same as training)
...
def load_model(checkpoint_path):
    ...
    actor = Actor(state_dim=18, action_dim=4).to(device)
...
def test_model(actor, num_episodes=20, max_steps=1000, visualize=True):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test TD3 Crazyflie Model')
    ...
    try:
        ...
        if args.model:
            ...
            # Look for best TD3 model first
            if os.path.exists('models/crazyflie_td3_best.pth'):
                checkpoint_path = 'models/crazyflie_td3_best.pth'
            else:
                # Find latest TD3 checkpoint
                td3_checkpoints = [f for f in os.listdir('models') 
                                 if f.startswith('crazyflie_td3_checkpoint_') and f.endswith('.pth')]
                if td3_checkpoints:
                    latest = max(td3_checkpoints, key=lambda x: int(x.split('_')[-1].split('.')[0]))
        ...
        actor = load_model(checkpoint_path)
        if actor is None:
            return
        
        print(f"\nTesting TD3 model with {args.episodes} episodes...")
        print("This will test across multiple difficulty levels")
        
        # Start test in separate thread
        test_thread = threading.Thread(
            target=test_model, 
            args=(actor, args.episodes, 1000, not args.no_viz),
            daemon=True
        )
        test_thread.start()
